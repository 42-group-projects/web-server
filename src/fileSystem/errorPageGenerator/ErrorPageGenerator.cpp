...
static const std::string errorPageTemplate =
	"<!DOCTYPE html>\n"
	"<html>\n"
	"<head><title>ERROR_MSG</title></head>\n"
	"<body><h1>ERROR_MSG</h1></body>\n"
	"</html>\n";

const char* ErrorPageGenerator::statusCodeToString(int code)
{
    switch (code)
    {
        case 0:   return "Unset";
...
ErrorPageGenerator::ErrorPageGenerator(e_status_code code)
{
	...
	{
		html.replace(pos, 9, statusCodeToString(code));
		pos = html.find("ERROR_MSG");
	}
}

const std::string ErrorPageGenerator::getHtml() const { return html; }